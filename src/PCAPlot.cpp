...
//
//  This is a OpenGL program for plotting the PCA analysis of different samples. A third dimension was added in order to clearly see the plots for each sample.
//	The code is based on Mike Bailey's OpenGL sample code.
//
...
const char *WINDOWTITLE = { "PCA Plot" };
...
void	Animate( void );
void	Buttons( int );
void	Spinners(int id);
void	Display( void );
void	DoRasterString( float, float, float, char * );
void	DoRasterString10( float, float, float, char * );
...
//alloc'd at startup by num samples
struct node *Nodes;
...
Display( void )
{
	...
	DoRasterString( 5., 1., 0., "Z Axis: Sample Name" );
...
InitGlui( void )
{
	...
	Max_Stdev = Glui->add_spinner("Max Fpkm Deviation", GLUI_SPINNER_FLOAT, &STDEVMAX, STDEV, (GLUI_Update_CB) Spinners);
	Max_Stdev->set_float_limits(0.0, 30., GLUI_LIMIT_CLAMP);
	Max_Stdev->set_speed(.75);
	...
	switch (id){
		case STDEV:
			for (int i = 0; i < TOTAL_NODES; i++){
					...
					float hue;
					if(Nodes[i].stdev >= STDEVMAX){
						hue = 0.;
					} else {
						hue =  140 - 140 * (Nodes[i].stdev - 0.)/(STDEVMAX);
					}
...
//gets the sample names from the command line, then cleans up the arguments for glut.
//right now it does not support glut arguments.
void
getSamples(int* argc, char** argv)
{
	if (*argc < 2){
		printf("Error: No samples provided\n");
		exit(-1);
	}
	NUM_SAMPLES = *argc;
	//allocate array
...
BuildSamples()
{
	...
	for (int i = 0; i < NUM_SAMPLES; i++){
		...
		while(fgets(buf, 50, fs) != NULL){
			loc_fpkm = atof(strtok(buf, " "));
			loc_stdev = atof(strtok(NULL, " "));
			...
			if(loc_CV < min_cv)
				min_cv = loc_CV;	
			Nodes[tot_samples].fpkm = loc_fpkm;
			Nodes[tot_samples].CV = loc_CV;
			Nodes[tot_samples].sample = i;
			Nodes[tot_samples].stdev = loc_stdev;
			tot_samples++;
			memset(buf, 0, 50);
		}
	}
	for (int i = 0; i < tot_samples; i++){
		...
		//start_point + (end_point - start_point)*value/maximu
		//float hue = 240. - 240 * (t - TEMPMIN)/(TEMPMAX-TEMPMIN);

		// max - min * (value - valmin / valmax - valmin )- max
		float hue;
		if(Nodes[i].stdev >= STDEVMAX){
								hue = 0.;
		} else {
...
InitLists( void )
{
	...
	if (MODE == db){
		...
		sprintf(bufx, "%f", curx);
		sprintf(bufy, "%f", cury);
		curx = 2 * (curx - min_fpkm) / (max_fpkm - min_fpkm) - 1;
		cury = 2 * (cury - min_cv) / (max_cv - min_cv) - 1;
		DoRasterString10( curx, 2., 0., bufx );
		DoRasterString10( 2., cury, 0., bufy );
		for(int i = 1; i < 4; i++){
			curx = ((max_fpkm - min_fpkm) / 3 ) * i + min_fpkm;
			cury = ((max_cv - min_cv) / 3 ) * i + min_cv;
			sprintf(bufx, "%f", curx);
			sprintf(bufy, "%f", cury);
			curx = 2 * (curx - min_fpkm) / (max_fpkm - min_fpkm) - 1;
			cury = 2 * (cury - min_cv) / (max_cv - min_cv) - 1;
